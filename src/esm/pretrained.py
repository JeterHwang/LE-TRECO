...
def esm1_t6_43M_UR50S(ckpt_path=None):
    """6 layer transformer model with 43M params, trained on Uniref50 Sparse.

    Returns a tuple of (Model, Alphabet).
    """
    return load_model_and_alphabet("esm1_t6_43M_UR50S" if ckpt_path is None else ckpt_path)
...
def esm2_t12_35M_UR50D(ckpt_path=None):
    """12 layer ESM-2 model with 35M params, trained on UniRef50.

    Returns a tuple of (Model, Alphabet).
    """
    return load_model_and_alphabet("esm2_t12_35M_UR50D" if ckpt_path is None else ckpt_path)


def esm2_t30_150M_UR50D(ckpt_path=None):
    """30 layer ESM-2 model with 150M params, trained on UniRef50.

    Returns a tuple of (Model, Alphabet).
    """
    return load_model_and_alphabet("esm2_t30_150M_UR50D" if ckpt_path is None else ckpt_path)


def esm2_t33_650M_UR50D(ckpt_path=None):
    """33 layer ESM-2 model with 650M params, trained on UniRef50.

    Returns a tuple of (Model, Alphabet).
    """
    return load_model_and_alphabet("esm2_t33_650M_UR50D" if ckpt_path is None else ckpt_path)
